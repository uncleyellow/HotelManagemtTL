...
namespace HotelManagementDA.Controllers
{
    ...
    [ApiController]
    public class BillDetail : ControllerBase
    {
        private readonly IConfiguration _configuration;
        public BillDetail(IConfiguration configuration)
        {
        ...
        {
            string query = "SELECT id, name, email, phoneNumber, checkInDate, checkOutDate, kindOfRoom, roomNumber, price, description, status FROM billDetails";
        ...
        {
            string query = $@"UPDATE billDetails
        ...
        {
            string query = $@"DELETE FROM [dbo].[billDetails] WHERE [id] = '{id}'";
            QuerryExtension.ExecuteNonQuery(query);
        ...
        {
            string query = $@"DELETE FROM [dbo].[billDetails]  ";
            QuerryExtension.ExecuteNonQuery(query);
        ...
        {
            string query = $@"Select id, name, email, phoneNumber, checkInDate,checkOutDate, kindOfRoom, roomNumber, description, status, price from billDetails WHERE id = '{id}'";
            DataTable table = QuerryExtension.ExecuteQuery(query);
        ...
        {
            string query = $@"Select id, name, email, phoneNumber, checkInDate,checkOutDate, kindOfRoom, roomNumber, description, status, price from billDetails letter name LIKE '%{keyword}%'";
